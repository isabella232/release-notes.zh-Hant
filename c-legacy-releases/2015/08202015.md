...
* 更新了分類規則產生器的[「選取報表套裝和變數」](https://marketing.adobe.com/resources/help/zh_TW/reference/?f=classification_rule_definitions)面板。您可以用此面板新增報表套裝和變數至現存規則集。之前，變數和報表套裝是在建立規則集時指定。這項變更可避免分類變數發生已回報的問題。
...
* 如果使用 Amazon S3 傳送資料，新實施的 [!UICONTROL BucketOwnerFullControl] 功能會指定跨帳戶上傳的權限，因此使用者能在貯體中建立屬於完全不同帳戶的物件。Amazon Web Services (AWS) 帳戶擁有者建立貯體，並建立擁有在貯體中建立物件權限的使用者，接下來提供其他使用者建立物件的認證。如需此功能的完整資訊，請參閱[設定資料饋送](https://marketing.adobe.com/resources/help/zh_TW/sc/clickstream/?f=datafeeds_configure)。
...
* 修正錯誤，此錯誤會讓[!UICONTROL 訪客 ID 服務]無法正常設定多部分頂級網域的 Cookie。例如，假設您有一個 ` my_company.co.uk` 網域，在某些情況下，[!UICONTROL 訪客 ID 服務]只會以 ` co.uk` 設定 Cookie。(AN-104683)
   ...
   * 使用訪客 ID 服務。
   * 啟用[寬限期](https://marketing.adobe.com/resources/help/zh_TW/mcvid/?f=mcvid_grace_period.html)，*或*使用第一方 Cookie 且使用者封鎖第三方 Cookie。
   * 擁有的頁面具有多部分的頂級網域。
...
* 新文件將說明如何使用[!UICONTROL 批量管理工具]。如需詳細資訊，請參閱[大量管理工具](https://marketing.adobe.com/resources/help/en_US/aam/?f=c_bulk.html)。
* 修訂[主要行銷描設定整合](https://marketing.adobe.com/resources/help/en_US/aam/?f=c_mmp.html)以更新上下文資料變數。此變數應該以底線作為結尾，但之前是顯示以句點作為結尾。

**已知問題**

[!DNL  Analytics] 客戶使用[主要行銷設定檔](https://marketing.adobe.com/resources/help/en_US/aam/?f=c_mmp.html)時需要設定整合代碼，並將宣告 ID 透過[訪客 ID 服務](https://marketing.adobe.com/resources/help/zh_TW/mcvid/?f=mcvid_service.html)傳送至 [!DNL  Audience Manager]。如需詳細資訊，請參閱[允許客戶合併設定檔](https://marketing.adobe.com/resources/help/en_US/aam/?f=c_profile_merge_enablement.html)。
