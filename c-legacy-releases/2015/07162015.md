...
* 修正「檢視/編輯結構」頁面 (客戶屬性中) 上指定的屬性說明無法在 Analytics 報表中更新的問題。(MAC-25985)
...
已知問題會導致在Audience Manager中建立重複的自動產生帳戶，並自動將其連結至使用者的Marketing Cloud身分。 如果嘗試在連結帳戶前導覽至 Audience Manager，就會發生此問題。Adobe建議您先將Audience Manager帳戶連結至Marketing Cloud，然後再導覽至Audience Manager。 (MAC-25640)
...
<table id="table_7CE66B92C8C1407185AB0E42DCB4972F"> 
 ...
 <tbody> 
  <tr valign="top"> 
   <td colname="col1"> <p>Event 類型 </p> </td> 
...
* [此項目中的新功能: [!DNL Analytics]](../../c-legacy-releases/2015/07162015.md#features_analytics)
* 修正:
...
<table id="table_91D1FD20C4C1411292252364328677AF"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> Analytics管理控制台中群組權限設定的變更 </td> 
  ...
  <tr> 
   <td colname="col1"> <p>Analytics LiveStream點擊平滑 </p> </td> 
   <td colname="col2"> <p> 點擊平滑化會緩衝資料，以提供訊息的平滑流量。 點擊平滑功能在數種情況下都很實用，例如傳入即時控制面板時會出現突發或無法預測。 在這種情況下，讓資料平順化可讓更新更順暢。 </p> <p> 若要啟用「點擊平穩化」，請將查詢字串參數 <span class="codeph">"smoothing=1"</span> 新增至 LiveStream 連接 URI 的尾端。例如，如果 LiveStream 連接 URI 為: </p> <p> 
     <codeblock>
  ...
  <tr> 
   ...
   <td colname="col2"> <p><span class="wintitle">提升度和可信度</span>報表已新增至 Analytics 和 Target Standard。<a href="https://docs.adobe.com/content/help/en/analytics/components/variables/dimensions-reports/report-target-lift-confidence.html" format="https" scope="external"> 更多內容... </a></p> <p>我們還將維度和量度從 <span class="keyword">Target</span> 新增 Ad Hoc Analysis。 </p> <p>您將能夠以過去在Target Classic中所做的方式，在Adobe Analytics中評估促銷活動的成功。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> Report Builder 關聯劃分功能表 </td> 
   <td colname="col2"> <p>在具有Analytics維度值的儲存格上按一下滑鼠右鍵時，新增「劃分」功能表旁的個別關聯劃分。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>移除封鎖案頭瀏覽器上所有Cookie的使用者</p> </td> 
   <td colname="col2"> <p> 我們已更新<span class="wintitle">隱私權設定</span>頁面上描述的使用範本區段的介面指示: </p> <p> <span class="uicontrol"> 管理工具</span> &gt; <span class="uicontrol">報表套裝</span> &gt; <span class="uicontrol">編輯設定</span> &gt; <span class="uicontrol">一般</span> &gt; <span class="uicontrol">隱私權設定 </span></p> <p> 此<span class="term">移除已封鎖桌面瀏覽器上所有 Cookie 的使用者</span>設定會讓 Adobe Analytics 無法報告封鎖桌面瀏覽器上所有 Cookie 的網站訪客。當作用中時，Analytics報表和專案會排除從不接受Cookie的案頭瀏覽器收集到的資料。 如果Adobe無法辨識瀏覽器，Analytics報表將包含其資料。 範本區段可用來檢視啟用此設定時報表的外觀。 </p> <p> <a href="https://docs.adobe.com/content/help/en/core-services/interface/ec-cookies/browser-cookie-settings.html" format="https" scope="external"> 更多內容... </a> </p> </td> 
  </tr> 
 </tbody> 
</table>
...
* 修正在多個報表套裝中使用區段時發生的問題。 (AN-104828)
* 變更在「管理控制台」中可顯示的報表套裝數目限制，同時仍顯示500到400的自訂欄。 (AN-102864)
* 已修正內容感知服務的問題 - 欄大小已增加至 255 以允許 UTF-8 編碼的資料。(AN-103834)
* 修正「自訂轉換」報表功能表未載入UI的問題。(AN-103816)
* 已修正未以 [!DNL  .pdf] 格式產生轉換報表的問題。(AN-103037)
* 修正某些報表中的圖形顯示未來月份資料的問題。 （AN-104372和AN-101257）
* 修正「顯示區段」未載入控制面板報表的問題。 (AN-105088)
* 修正控制面板顯示無法讀取的行項目的問題。 行項目在多位元組字元的中間被截斷。 (AN-104940)
* 修正自訂日曆無法傳回正確資料的問題。 (AN-103825)
* 修正PDF下載無法使用時間格式來處理量度的問題。 (AN-101515)
* 修正包含區段的計算量度傳回錯誤資料的問題。 (AN-105320)

### Report Builder 修正 {#fixes_arb}

* 修正導致「總分鐘數」計算量度未顯示在使用者介面中的問題。 (AN-105218)

### Ad Hoc Analysis 修正 {#fixes_ad_hoc}

* 修正「城市」報表（地域劃分）中遺失「城市」資料的問題。 (AN-104987)
* 已修正轉換量度無法正確載入的問題。 (AN-104957)
* 修正在建立計算量度時使用4個以上數學運算，導致錯誤的問題。 (AN-105362)

### Data Warehouse 修正 {#fixes_data_warehouse}

* 修正區段的變數／日期範圍支援。 (AN-105271)
* 修正無法產生UTF-8字元的Tableau檔案的問題。 (AN-75471)
* 修正每次點擊具有多個產品的報表上的計數器事件度量去重複化，以及產品分類的劃分。
* 「Data Warehouse」中「頁面檢視」量度報告的結果，現在將完全歸納在「Reports &amp; Analytics」結果中。(AN-97619)

### 點按資料流饋送修正 {#clickstream}

**更新 (2015 年 7 月 27 日)**: 我們正在更新資料饋送的傳送基礎結構。我們正在新增額外的IP位址至已用於傳送資料饋送的儲存區。 如果您的FTP網站上有資料饋送傳送至的IP白名單，您需要將這些額外的IP位址新增至該白名單。
...
* 資料來源API方法已移轉至Swagger。 See the [Audience Manager API Docs](https://bank.demdex.com/portal/swagger/index.html) for methods, syntax, and code samples.
