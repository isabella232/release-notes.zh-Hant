...
* **Dynamic Media 新增 3D 資產支援**
   ...
   * 若需 AEM as a Cloud Service 的相關資訊，請參閱[在 Dynamic Media 中使用 3D 資產](https://docs.adobe.com/content/help/zh-Hant/experience-manager-cloud-service/assets/dynamicmedia/assets-3d.html)。
...
| 2020 年 6 月 23 日 | [處理設定檔](https://docs.adobe.com/content/help/en/experience-manager-learn/assets/configuring/processing-profiles.html) | 影片 | 處理設定檔能針對在 AEM as a Cloud Service 中所建立資產，定義轉譯作業。 |
...
| 2020 年 6 月 23 日 | [針對 AEM as a Cloud Service 的建立及部署工作除錯](https://docs.adobe.com/content/help/en/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/build-and-deployment.html) | 文章 | 了解如何針對AEM as a Cloud Service 的建立及部署工作除錯。 |
| 2020 年 6 月 16 日 | [以記錄檔為 AEM as a Cloud Service 除錯](https://docs.adobe.com/content/help/en/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/logs.html) | 文章 | 了解如何使用記錄檔為 AEM as a Cloud Service 除錯。記錄檔是 AEM 應用程式除錯作業的最前線，但部署的 AEM 應用程式必須有充足的登入次數。 |
...
| 功能 | 說明 |
| -----------| ---------- |
| GDPR透明度與同意框架 | Advertising Cloud將在2020年8月15日之前將對IAB透明度與同意框架的支援升級為2.0版。 目前，客戶端不需要進行任何更改。  作為每個客戶的資料處理者，Adobe Advertising Cloud有能力讓客戶履行GDPR中的義務，這些義務適用於我們的產品和服務的使用。 |
| Audience Manager整合 | (僅限Adobe Experience Platform的Adobe Audience Manager客戶測試版)DSP可以透過Adobe Experience Platform將包含廣告商廣告參與資料的可操作記錄檔傳送至Audience Manager。 在Audience Manager中，廣告商可以使用資料來進行進階細分，並在前瞻分析工具(例如Audience Optimization報表)中使用。 |
| 以人為本的跨裝置圖形定位 | 自助式客DSP戶可以使用裝置圖表(AdobeDevice Co-op或LiveRamp)，針對新促銷活動進行以人為本的鎖定和頻率管理。 這可確保廣告商可透過其擁有的裝置觸及其受眾，但限制曝光度。 此外，跨位置大量編輯跨裝置定位設定現在是測試版。 若要啟用您帳戶的測試版，請連絡您的Adobe帳戶管理員。 |
| 網站定位 | 「黑名單」和「白名單」已更名為「blocked(sites)」和「targeted(sites)」等更公平的詞語。 |
| Flashtalking廣告 | 現在DSP，透過單一廣告建立和大量協力廠商廣告建立工作流程，自動插入所有Flashtalking廣告標籤的點按巨集。 |
| 報表 | (僅適用於Adobe Experience Platform客戶的測試版)Advertising Cloud Search的全新跨通道報告提供媒體支出與搜尋的統一檢視DSP，以及位置與搜尋促銷活動的每DSP日關鍵量度。 |
