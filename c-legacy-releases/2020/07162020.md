...
| 功能 | 說明 |
| -----------| ---------- |
| GDPR透明度與同意框架 | Advertising Cloud將於2020年8月15日將對IAB的「透明度與同意框架」的支援升級至2.0版。 目前，客戶端不需要進行任何更改。  Adobe Advertising Cloud是我們每位客戶的資料處理者，可讓客戶履行GDPR規定的義務，而GDPR規定的義務適用於我們產品和服務的使用。 |
| Audience Manager整合 | （僅限Adobe Experience Platform的Adobe Audience Manager客戶測試版）DSP可以透過Adobe Experience Platform將包含廣告商廣告參與資料的可操作記錄檔傳送至Audience Manager。 在Audience Manager中，廣告商可以使用資料，在深入解析工具（例如「對象最佳化報表」）中進行進階細分和使用。 |
| 以人為本的跨裝置圖形定位 | 自助式DSP客戶可使用裝置圖表（Adobe Device Co-op或LiveRamp），針對新宣傳活動進行以人為本的鎖定和頻率管理。 這可確保廣告商可透過其擁有的裝置觸及其受眾，但限制曝光度。 此外，跨位置大量編輯跨裝置定位設定現在是測試版。 若要啟用您帳戶的測試版，請連絡您的Adobe客戶經理。 |
| 網站定位 | 「黑名單」和「白名單」已更名為「blocked(sites)」和「targeted(sites)」等更公平的詞語。 |
| Flashtalking廣告 | DSP現在可透過單一廣告建立和大量協力廠商廣告建立工作流程，自動插入所有Flashtalking廣告標籤的點按巨集。 |
| 報表 | （僅適用於Adobe Experience Platform客戶的測試版）Advertising Cloud Search中的全新跨通道報告提供DSP和搜尋媒體支出的統一檢視，以及DSP位置和搜尋促銷活動的每日關鍵指標。 |
