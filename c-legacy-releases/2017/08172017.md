...
8 月 22 日起，Adobe Analytics 將啟用[地域劃分](https://marketing.adobe.com/resources/help/zh_TW/reference/reports_geosegmentation.html)報表供所有客戶使用。此次變更是為了因應要在近期移除 Admin Console 的地域劃分設定所做。
...
* 與[組織 ID](https://marketing.adobe.com/resources/help/en_US/aam/ids-in-aam.html) 連結的使用者現在可以使用其 Adobe ID 登入 Audience Manager UI，無需再透過使用者名稱和密碼。(AAM-28161)
...
[DIL](https://marketing.adobe.com/resources/help/en_US/aam/dil-overview.html) 是 Audience Manager 的 API 資料收集程式碼。8 月發行版本將 DIL 推進至 6.11 版。
...
我們已重新修訂關於如何使用資料收集伺服器 (DCS) API 的文件。從 [DCS 伺服器 API 方法與程式碼](https://marketing.adobe.com/resources/help/en_US/aam/dcs-intro.html)開始，逐步使用 DCS API 完成更複雜的操作。
...
* [監聽規則](https://marketing.adobe.com/resources/help/en_US/social/c_settings_rules.html): 規則會在 6 個月後自動失效。您可以在[!UICONTROL 「規則產生器」]的[!UICONTROL 「非活動」]標籤裡找到過期的規則，且在 6 個月內均可進行個別或大量更新。
