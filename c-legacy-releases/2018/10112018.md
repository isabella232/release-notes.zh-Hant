...
全新 [!DNL Analytics] API使用Adobe I/閘道，讓您輕鬆搭配任何其他Adobe API使用。API將相同的自訂層級延伸至客戶在 [!UICONTROL 分析工作區]中習慣的API。這種作法可真正達到 REST，讓開發人員可以更輕鬆的方式與 API 互動。
...
已套用至Adobe [!DNL Analytics] 介面工具和元件([!UICONTROL Analysis Workspace]、 [!UICONTROL 報告與分析]、 [!UICONTROL 報告建立工具]等)的修正和次要更新。
...
* **管理員**[!DNL Analytics]: 修正 使用者名稱中出現特殊字元時會發生問題的情況。Experience Cloud登入可讓您使用一些特殊字元。現在 [!DNL Analytics] 支援Experience Cloud使用者名稱中允許的所有字元。(AN-168014)
