...
### [!UICONTROL Mobile Services和Mobile SDK]{#mobile}
...
| 功能 | 說明 |
| -----------| ---------- |  
| 地區資料收集作業異動 | 為提升資料收集效能，所有剩餘客戶皆已轉換為地區資料收集 (RDC)，使收集作業盡可能貼近您的使用者。這能改善網站/應用程式效能，並確保資料能儘快收集齊全，落實使用者體驗最佳化。<br/>按地區收集您數位財產中的資料後，系統會透過安全連線將資料轉送到資料處理中心，資料經過處理後，便可供 Adobe Experience Cloud 的產品使用。十幾年來，新的實作項目一律預設採用 RDC。[更多](https://marketing.adobe.com/resources/help/en_US/reference/regional-data-collection.html) |
| 中國RDC的變更 | Adobe 在中國大陸實施地區資料收集作業，中國境內的客戶可將資料直接傳送至中國境內的資料收集中心，而非傳送至全球其他地點。比起將資料傳送至中國境外的資料收集中心，這麼做可縮短頁面載入時間，並提升資料準確度。[更多](https://marketing.adobe.com/resources/help/en_US/reference/rdc-china.html) |
| 中國地區加速 | 現在，Analytics 報表套裝設定包含「中國加速」選項。此選項可為中國大陸的使用者提升 Adobe Analytics 報表的使用者體驗。所有 Adobe Analytics 使用者都可以啟用或停用此選項。請勿為中國大陸境外的使用者選取此選項，這麼做會對效能產生負面影響。[更多](https://marketing.adobe.com/resources/help/en_US/sc/user/report_settings.html) |
| Livestream 新變數 | * **電信業者**：行動裝置的無線供應商。<br/>* **tnt：** Target中使用的測試和目標欄位。_actionId_ 與 _actionName_ 從未有值。<br/>* **TntAction：** Target中使用的增強測試與目標欄位。與 tnt 欄位不同之處在於，_actionId_ 和 _actionName_ 會設定值。 |
| API2.0：支援JWT驗證 | Adobe Analytics 2.0 API 現起支援 JWT 驗證。如需 JWT 驗證的詳細資訊，請前往：[https://github.com/AdobeDocs/analytics-2.0-apis/blob/master/jwt.md](https://github.com/AdobeDocs/analytics-2.0-apis/blob/master/jwt.md) |
...
* **分析工作區：** 修正無法在地圖視覺化中顯示任何資料的問題。(AN-170626)
* 修正 Analytics 中 (已移轉) 使用者姓名與連結至該使用者 Experience Cloud 的 Adobe ID 名稱不同的問題。若要使用Experience Cloud中的帳戶來更新Analytics名稱，請前往 **[！UICONTRL管理員** &gt; **!UICONTROL 使用者管理(舊版)**&gt;**!UICONTROL 編輯使用者** 並按一下使用者ID。接著，請按一下**!UICONTROL 「從 Experience Cloud 更新」**。(AN-172825)
...
* 修正在「選擇特徵至排除」頁面中，演算法模型中「 **[!UICONTROL 選擇全部]**[」核取方塊遺失的錯誤](https://experiencecloud.adobe.com/resources/help/en_US/aam/trait-exclusion-algo-models.html) 。(AAM-42462)
...
閱讀 [ID監控和黑名單](https://marketing.adobe.com/resources/help/en_US/aam/id-monitoring-blacklisting.html) 以瞭解DCS如何監控其接收並列入黑名單的ID，並列入短期高費率。
...
* 現在，您可複製貼上同一工作流程內的活動，或是相同 Campaign 例項其他工作流程的活動。如此一來，您就可以輕鬆複製整個工作流程或特定活動，並保留最初定義的設定。如需詳細資訊，請參閱[詳細文件](https://helpx.adobe.com/campaign/standard/automating/using/workflow-interface.html#duplicating-workflow-activities)。(CAMP-20014)
...
| 功能 | 說明 |
| -----------| ---------- |  
| 文件夾與搜尋促銷活動 | 在「競標單元詳細資料」中，所有作品集的「歷史」和「預測」標籤現在包含「電腦」和「平板電腦」的個別索引標籤。以前，只有使用最佳化功能v的組合標籤才能使用個別標籤。<br/> 對於使用v的文件夾，「電腦」和「平板電腦」標籤的所有資料都是分開的。<br/>但是，對於使用最佳化功能v6的作品集，「電腦」和「平板電腦」索引標籤會顯示電腦和平板電腦的總和總計，這是先前為電腦+平板電腦顯示的項目。<br/>**注意：** 每當最佳化方法從v變更為v或相反時，歷史資料就會重設當天。 |
| 搜尋促銷活動 | 您現在可以啓用和停用多個搜尋引擎帳戶。<br/>(Bing Ads帳戶)您現有的Bing Ads自訂對象現在已包含在「對象&gt;程式庫」檢視中，並可作為Bing Ads廣告群組的目標。您可以選擇性地為每個對象目標設定競標修飾元。 |
