...
如需產品文件，請參閱 [Experience Cloud ID 服務](https://docs.adobe.com/content/help/en/id-service/using/home.html)。
...
| 中國的 RDC 異動 | Adobe在中國大陸的地區資料收集服務可讓中國境內客戶直接將資料傳送至中國境內的資料收集中心，而非全球其他地點。 相較於將資料傳送至中國境外的資料收集中心，這可改善頁面載入時間和資料準確度。 [更多內容](https://marketing.adobe.com/resources/help/zh_TW/reference/rdc-china.html) |
| 中國區域加速 | 「Analytics報表套裝設定」現在提供「中國加速」選項。 這個選項可改善中國大陸使用者的Adobe Analytics報告使用體驗。 每個Adobe Analytics用戶都可以啟用或禁用此選項。 請勿針對中國大陸以外的使用者選取此選項，因為這會對效能產生負面影響。 [更多內容](https://marketing.adobe.com/resources/help/zh_TW/sc/user/report_settings.html) |
...
* 修正 Analytics 中 (已移轉) 使用者姓名與連結至該使用者 Experience Cloud 的 Adobe ID 名稱不同的問題。若要使用Experience Cloud中的Analytics名稱更新，請前往「**[!UICONTROL Admin]** > **[!UICONTROL 使用者管理（舊版）]** > **[!UICONTROL 編輯使用者]**」，然後按一下使用者ID。 然後，按一下「從Experience Cloud ]**更新」。**[!UICONTROL (AN-172825)
...
| 功能 | 說明 |
| -----------| ---------- |  
| [特徵建議](https://experiencecloud.adobe.com/resources/help/en_US/aam/trait-recommendations.html) | 特質Recommendations由[Adobe Sensei](https://www.adobe.com/tw/sensei.html)提供支援，將資料科學帶入您的Audience Manager日常工作流程。 <br/>有了特徵建議功能，您在[區段產生器](https://experiencecloud.adobe.com/resources/help/en_US/aam/t_create_segment.html)中建立或編輯區段時，系統會根據區段規則中的特徵，提供您其他類似特徵的相關建議，您可考慮納入。將建議的特徵加入區段，有助於擴大您的目標對象範圍。 |
| [全域資料來源](https://marketing.adobe.com/resources/help/en_US/aam/global-data-sources.html) | 全球資料來源可供所有Audience Manager客戶存取，並包含由裝置製造商（例如Apple、Samsung、Microsoft、Roku和Android裝置製造商）產生的裝置廣告ID。 這些ID由製造商提供，以供廣告之用。 您可以使用全球資料來源同步裝置 ID，並匯入或匯出這些對應所中斷的資料。<br/>Audience Manager 會根據格式，驗證客戶匯入的裝置廣告 ID (DAID)，確保符合裝置製造商列出的標準格式。 |
...
本版本包含Adobe Campaign與Adobe Experience Platform Launch和行動SDK中Adobe Campaign Standard通用可用(GA)版Android和iOS擴充功能的整合。 這些擴充功能支援推播訊息、應用程式內訊息和行動應用程式設定檔更新。

如需詳細資訊，請參閱[使用Adobe Experience PlatformSDK](https://helpx.adobe.com/tw/campaign/standard/administration/using/configuring-a-mobile-application.html#main-pars_header_49)。

### 行動應用程式內訊息

此發行包含Campaign中的GA版應用程式內渠道。 從功能角度來看，測試版中最值得注意的新增功能是「應用程式內通道動態報表」，以及Mobile SDK與MCIAS(Marketing Cloud應用程式內訊息服務，為SDK提供應用程式內規則)之間的安全握手。 安全握手可確保使用者的PII資料不會落入惡意手中，並讓您在使用者每次登出時清除訊息快取，以維護共用裝置上的使用者隱私。

如需詳細資訊，請參閱[關於應用程式內訊息](https://helpx.adobe.com/tw/campaign/standard/channels/using/about-in-app-messaging.html)和專用的[應用程式內教學課程](https://helpx.adobe.com/tw/campaign/kt/acs/using/acs-in-app-message-tutorial.html)。
...
* 使用&#x200B;**載入檔案**&#x200B;活動時，您現在可以在包含拒絕記錄的檔案名稱中新增時間戳記。 如需詳細資訊，請參閱[詳細文件](https://helpx.adobe.com/tw/campaign/standard/automating/using/load-file.html#configuration)。
* **「查** 詢」和「 **** 區段」活動現在可讓您在活動未擷取任何資料時啟用傳出轉場。
...
內容與簡報中的一些改善項目相同。更令人興奮的是，新功能可讓您提供檔案建議。 您可以透過提取要求提交建議，或利用每個頁面上的&#x200B;**[!UICONTROL 編輯此頁面]**&#x200B;連結及&#x200B;**[!UICONTROL 記錄問題]**&#x200B;連結，在 GitHub 上提出問題。
...
如需最新資訊，請參閱[Livefyre發行說明](https://docs.adobe.com/content/help/en/livefyre/using/release-notes/c-rn.html)。
