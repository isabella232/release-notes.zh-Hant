...
# Adobe Experience Cloud發行說明- 2019年2月
...
| 功能 | 說明 |
| -----------| ---------- |  
| 世代分析的變更 | 我們對Choort Analysis的運作方式做了兩小幅變更，該分析於2019年1月發佈重大更新。 這些修改可能會變更傳回的資料。<ul><li>「自訂同類群組Dimension」區段定義現在只會將維度項目套用為包含期間的一部分，而不會套用為退貨定義的一部分。</li><li> 對於滾動計算，我們將在每個後續返回期間應用包含標準。 現在，返回標準會改為套用至選定期間之前的每個期間。</li></ul> |
| 功能 B | 文字 |
| 功能 C | 文字 |

#### 世代分析的變更

我們對Choort Analysis的運作方式做了兩小幅變更，該分析於2019年1月發佈重大更新。 這些修改可能會變更傳回的資料。

* 「自訂同類群組Dimension」區段定義現在只會將維度項目套用為包含期間的一部分，而不會套用為退貨定義的一部分。
* 對於滾動計算，我們將在每個後續返回期間應用包含標準。 現在，返回標準會改為套用至選定期間之前的每個期間。
...
此功能可讓企業衡量觸及的聽眾範圍和內容受歡迎的程度，並根據使用者行為和類似受眾模型，提供讓使用者持續參與的深入分析。使用我們同級最佳的SDK評估10秒音訊內容和1秒串流廣告，以評估體驗品質（停頓、錯誤、開始時間）和內容加上廣告KPI。

**維護廣告插播**

在播放器已輸入廣告插播，且特定時段內未傳送呼叫的情況下，增強廣告追蹤，SDK邏輯會預設為內容播放，即使播放器仍處於廣告插播狀態亦然。 在前段廣告期間，這可能會在廣告中斷完成前引發內容開始引發。 在Media 2.2 SDK中，當廣告之間有長時間延遲時，會在廣告中斷內的廣告之間傳送緩衝呼叫。

**SDK 名稱變更**

從2.2版開始，視訊心率程式庫(VHL)SDK會重新命名為Media SDK。 Media 2.2 SDK完全向後相容於VHL 2.X SDK系列。 名稱變更不代表功能中斷，只代表命名慣例的變更。
...
這項變更是為了在量度和報告中反映業界標準，並讓量度在報告中更容易識別。 新量度名稱將立即顯示在Analytics報表中，不需要代表使用者進行變更。
...
我們對Choort Analysis的運作方式做了兩小幅變更，該分析於2019年1月發佈重大更新。 這些修改可能會對傳回的資料進行小幅變更。
...
* 已針對Analysis Workspace的特定錯誤新增一些重試邏輯。 (AN-172801)
...
升級至新版[Tableau](https://experiencecloud.adobe.com/resources/help/en_US/aam/c_comproc.html)後，我們變更了重疊報表的資料視覺化方法。 請參閱特徵對特徵重疊報表中的螢幕截圖。請參閱[特徵對特徵重疊報表](https://experiencecloud.adobe.com/resources/help/en_US/aam/c_overlap_reports.html)。

**區段對特徵重疊報表**

升級到 Tableau 的新版本後，我們變更了重疊報表的資料視覺效果方法。請參閱區段對特徵重疊報表中的螢幕截圖。請參閱[區段對軌重疊報表](https://experiencecloud.adobe.com/resources/help/en_US/aam/c_segment_trait_overlap.html)
...
AEM 6.1 SP2-CFP18(6.1.2.18)是2019年1月11日發行的重要更新，其中包含自2016年8月AEM6.1日全面推出以來的重要客戶修正。
...
整合問題AEM的疑難排解提示，請參閱[整合問題疑難排解](https://helpx.adobe.com/experience-manager/6-4/sites/administering/using/troubleshooting-integration-issues.html)。
...
（測試版）新的「每週」支出策略可讓您在嘗試花費指定每週金額的同時，將加權收入最大化。 這個策略會自動使用星期幾模式與模擬。如果您想要將此策略用於投資組合，請等到該投資組合最佳化至少兩週，然後主動監控支出和效能，直到該功能被驗證為止。
...
[廣告雲端社群](https://forums.adobe.com/community/experience-cloud/advertising-cloud)
