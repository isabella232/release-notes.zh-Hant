...
| 功能 | 說明 |
| -----------| ---------- |  
| 世代分析的變更 | 我們對Choort Analysis的運作方式做了兩小幅變更，該分析於2019年1月發佈重大更新。 這些修改可能會變更傳回的資料。<ul><li>「自訂同類群組維度」區段定義現在只會將維度項目套用為包含期間的一部分，而不會套用為退貨定義的一部分。</li><li> 對於滾動計算，我們將在每個後續返回期間應用包含標準。 現在，返回標準會改為套用至選定期間之前的每個期間。</li></ul> |
| 功能 B | 文字 |
| 功能 C | 文字 |
...
* 「自訂同類群組維度」區段定義現在只會將維度項目套用為包含期間的一部分，而不會套用為退貨定義的一部分。
...
With the upgrade to a new version of [Tableau](https://experiencecloud.adobe.com/resources/help/en_US/aam/c_comproc.html), we have changed the data visualization method for overlap reports. 請參閱特徵對特徵重疊報表中的螢幕截圖。See [Trait-to-Trait Overlap Report](https://experiencecloud.adobe.com/resources/help/en_US/aam/c_overlap_reports.html).

**區段對特徵重疊報表**

升級到 Tableau 的新版本後，我們變更了重疊報表的資料視覺效果方法。請參閱區段對特徵重疊報表中的螢幕截圖。請參 [閱區段對軌重疊報表](https://experiencecloud.adobe.com/resources/help/en_US/aam/c_segment_trait_overlap.html)
...
AEM 6.1 SP2-CFP18(6.1.2.18)是2019年1月11日發行的重要更新，其中包含自2016年8月AEM 6.1全面推出以來發行的重要客戶修正。
...
AEM整合問題的疑難排解提示，請參閱「疑 [難排解整合問題」](https://helpx.adobe.com/experience-manager/6-4/sites/administering/using/troubleshooting-integration-issues.html)。
