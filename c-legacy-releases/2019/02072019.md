...
視訊和音訊的Analytics中的新功能

**媒體開始次數 - 量度名稱變更**

 在 2019 年 2 月 7 日，針對視訊和音效的 Adobe Analytics 將發佈量度名稱變更。媒體起始將更名為_媒體開始次數_。
...
* **報告與分析：** 修正使用者無法與其他使用者共用報告的問題。(AN-172301)
* **管理員：** 修正舊版管理員使用者管理中的編輯功能問題：選取使用者並按一下「編輯」後，不會顯示有關使用者的資料。(AN-170705)
* 為 Analysis Workspace 中的特定錯誤新增一些重試邏輯。(AN-172801)

### JavaScript適用的AppMeasurement{appm}

JavaScript2.11.0-月11日發行
...
升級到 [Tableau](https://experiencecloud.adobe.com/resources/help/en_US/aam/c_comproc.html) 的新版本後，我們變更了重疊報表的資料視覺效果方法。請參閱特徵至特徵重疊報表中的螢幕截圖。請參閱 [「特徵對特徵重疊報表](https://experiencecloud.adobe.com/resources/help/en_US/aam/c_overlap_reports.html)」。

**區段至特徵重疊報表**

升級到 Tableau 的新版本後，我們變更了重疊報表的資料視覺效果方法。請參閱區段至特徵重疊報表中的螢幕截圖。請參閱 [區段對趨勢重疊報表](https://experiencecloud.adobe.com/resources/help/en_US/aam/c_segment_trait_overlap.html)

**區段至區段重疊報表**

升級到 Tableau 的新版本後，我們變更了重疊報表的資料視覺效果方法。請參閱區段至區段重疊報表中的螢幕截圖。請參閱 [區段對區段重疊報表](https://experiencecloud.adobe.com/resources/help/en_US/aam/c_segment_segment_overlap.html)
...
* 修正搜尋詞結尾為反斜線字元時，「特徵」、「區段」、「目的地」和「模型」搜尋欄位不顯示任何結果的問題。
* 修正了導致在 Feed 使用狀況報表的「應付款」部分中顯示不正確日期的問題。

## Experience Manager {#aem}

Adobe Experience Manager中的新功能、修正和更新。Adobe 建議使用內部部署的客戶部署最新修補程式，以確保擁有更出色的穩定性、安全性及效能。
