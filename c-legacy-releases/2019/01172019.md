...
[!DNL Experience Cloud] 介面的發行說明，包括 [!UICONTROL 平台] 核心服務和產品管理。
...
Experience Cloud介面中的功能和修正。

#### 版本 19.1.1

**注意：** 在2019年月，Experience Cloud介面將不支援Internet Explorer11。
...
[!DNL Experience Cloud] ID服務的改進和修正。
...
### [!UICONTROL Mobile Services] 和Mobile SDK {#mobile}
...
此設定可透過 **[!UICONTROL 「專案]** &gt; **[!UICONTROL 專案資訊與設定]**」存取。
...
* 管理員：修正即使未指派適當權限，非管理員仍無法建立處理規則的問題。(AN-170690)
...
| 資料保留: 檢查並設定 Adobe Analytics 的資料保留政策 | 2018 年 2 月 1 日 | **背景：** 歐盟通用資料保護規則(GDPR)自2018年月25日起施行，提供Adobe作為資料處理方，必須採取適當措施協助其客戶履行存取、刪除及其他要求。套用適當、安全與即時的刪除政策，是遵守這項法規中非常重要的一個環節。因此，Adobe 想在 2018 年 5 月 25 日 GDPR 生效前，與您合作執行資料保留政策。<br/>**期望：** 除非您已制定Adobe Analytics資料保留政策，否則Adobe將開始針對Adobe Analytics中目前指定的客戶合約套用資料保留，除非進行其他安排。Adobe 可在 25 個月後刪除資料，大部份的 Adobe Analytics 合約中都已說明此事。一旦您的組織制定了資料保留政策，該政策就會每個月加以執行。資料保留時間若超過 25 個月，需額外收費。資料保留時間也可以透過聯絡客戶服務部門，設定為較短的保留時間。您很快就會收到您組織的額外詳細資料電子郵件。<br/>資料保留影響所有存取 Adobe Analytics 歷史資料的方式，其中包含但不限於 Reports &amp; Analytics、Analysis Workspace、Report Builder、Web Services Reporting Api、Data Warehouse 以及資料摘要。**後續步驟：** 識別負責對資料保留做出決策的組織中的利害關係人。您的組織最好瞭解 Adobe Analytics 資料應該保留的適當期限。如果您有任何與 Adobe Analytics 資料保留相關的問題，請聯絡您的 Adobe Customer Success Manager。 |
...
新功能和修正 [!DNL Adobe Audience Manager]。

### 資料整合程式庫 (DIL) 8.1

在 8.1 版中，我們讓 DIL 遵循 Google Tag Manager 更嚴格的安全性原則。僅透過 DTM 提供此版本，且為獨立程式碼的形式。Launch 和 AppMeasurement Audience Manager 模組皆不提供。未來版本將會包含此一變更。(CORE - 24189)


### 修正in [!DNL Audience Manager]
...
適用於XML文件解決方案的3.2版已於2018年12月18日推出。
...
* 「開啟」區塊化時，會產生錯誤的 TOC。
* AEM XML 編輯器會弄亂橫跨的欄。
* 在符號中斷 `<li>` 的 `<p>` 開頭處使用Enter。
* 編輯主題，且使用者從「作者」模式切換至「來源」模式 (或相反) 時，程式碼區塊會縮排。
* 在「作者」模式中，如果使用Enter鍵新增表格儲存格中的段落 `<p>` 元素，則會造成PDF輸出中多餘的雜湊字元。
...
| 功能 | 說明 |
| -----------| ---------- |  
| 全面推出 Email Designer | 全新直觀式 Email Designer (原稱 Creative Designer) 現已全面推出。Email Designer 支援舊版內容編輯器的所有功能，包括:使用Adobe Target的動態影像；在準備時自動擷取URL內容的能力；完全相容的盒裝內容範本。如需詳細資訊，請參閱電子郵件內容設計和功能影片。因此，舊版的電子郵件內容編輯器目前已停用。如需詳細資訊，請參閱 Campaign Standard 中已不再使用和已移除的功能。 |
...
| 應用程式內傳訊功能 Beta 版改良 | 應用程式內傳訊功能 Beta 版已經過強化，現可提供下列新功能:應用程式內測試版與GDPR相容；與Analytics API整合以填入觸發器dps；傳遞範本的直覺式外觀和說明；製作介面從可用性觀點的增強功能。如需詳細資訊，請參閱詳細文件。 |
...
新功能 [!DNL Adobe Advertising Cloud]。
