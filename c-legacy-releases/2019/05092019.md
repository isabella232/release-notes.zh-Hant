...
| **Analysis Workspace:** 全新&#x200B;_包含重複例項_&#x200B;的流量視覺效果設定 | _「包含重複例項」_&#x200B;流量設定可讓您選擇納入或排除重複例項，例如「頁面重新載入」。此外，現在所有「流量」視覺效果皆僅根據例項。 |
| **Ad Hoc Analysis:** 與 Java 11 相容 | Ad Hoc Analysis 現在與 Java 11 相容。Learn how to run [Ad Hoc Analysis on Java 11](https://marketing.adobe.com/resources/help/en_US/dsc/adhoc-java.html). |
| **資料彙集:** 全新 s_ecid Cookie | Added a new [first-party server cookie](https://docs.adobe.com/content/help/en/core-services/interface/ec-cookies/cookies-analytics.html), s_ecid, in which data collection stores the visitor's ECID. |
...
| [!UICONTROL 「異常偵測」和「貢獻分析」這兩項功能已從 ]Reports &amp; Analytics 中移除 | 2018 年 4 月 10 日 | 「異常偵測」和「貢獻分析」這兩項功能已從 Reports &amp; Analytics 功能集中移除，現在只能透過 Analysis Workspace 使用。Adobe Analytics Select 與 Foundation 客戶在 Workspace 中只能存取「每日精細度」的「異常偵測」功能。 |
