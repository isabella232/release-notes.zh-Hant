...
* [!DNL Advertising Cloud](#adcloud) ( **更新日期：2020年2月10日)**
...
| 檢視 | 功能 |
|------|---------|
| 產品組合 | 您現在可以新增Yahoo! 日本顯示網路(YDN)促銷活動，以最佳化促銷活動預算和群組層級的競標。 相同的出價會套用至廣告群組中的所有廣告。 YDN促銷活動的資料會包含在產品組合的模擬中。 |
| 搜尋>大量工作表 | 您現在可以使用大量工作表建立、編輯和刪除Google自適應搜尋廣告(RSA)。 以前，支援僅透過「搜尋>促銷活動」中的標準促銷活動管理介面提供 |
| 搜尋>促銷活動、報表 | Google Ads的顯著度量「影響」。 (Abs. 上)%」和「Impr。 （排名最前）%」現在可用於所有基本報表和實體層級的促銷活動管理檢視，購物產品群組除外、「促銷活動每日曝光分享」和「關鍵字每日曝光分享」報表，以及標籤和限制檢視。 |
