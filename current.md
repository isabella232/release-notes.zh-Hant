...
| 搜尋>大量工作表 | 您現在可以使用大量工作表建立、編輯和刪除Google自適應搜尋廣告(RSA)。 以前，支援僅透過「搜尋>促銷活動」的標準促銷活動管 **[!UICONTROL 理介面]** 提供 **[!UICONTROL 。]** |
| 搜尋>促銷活動、報表 | Google Ads的顯著度量 `Impr. (Abs. Top) %` 和 `Impr. (Top) %` ，現在可在所有基本報表和實體層級的促銷活動管理檢視中使用，購物產品群組除外，在「促銷活動每日曝光分享 [!UICONTROL 」和「關鍵字每日曝光分享] 」報表，以及標籤和限制檢視中都可使用。 |
