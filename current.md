...
* [!DNL Analytics](#analytics) (**2019 年 8 月 20 日更新**)
...
| 功能 | 說明 |
| -----------| ---------- |  
| 支援 SameSite Cookie 設定 | [SaveSite Cookie設定](https://web.dev/samesite-cookies-explained) 會新增至Analytics設定的所有Cookie。此變更可讓您符合需有 SameSite Cookie 欄位的 Chrome 變更。Analytics Cookie 將預設為 `none`。如果您只使用第一方網域(例如 stats.domain.com)，您可以為第一方收集網域將 Adobe ClientCare 設定為 `lax`。 |
| Workspace: 將下拉式篩選器的項目上限從 50 個提升至 200 個 | 我們將下拉式篩選器中可放置的項目上限從 50 個提升至 200 個。此增強功能適合多種使用案例，例如將所有國家 (195) 新增至篩選器，或新增所有美國州和省 (52)。 |
| 歸因 IQ 已啓用 A4T 活動曝光數和活動轉換數等度量 | 我們已為歸因 IQ 啓用兩個 Analytics for Target (A4T) 量度：活動曝光數和活動轉換數。在過去，在分析工作區中，相較於「報告與分析」，這些量度會膨脹。經過此變更後，使用者現在可以套用「比照調整」歸因模型，使 Analysis Workspace 與 Reports &amp; Analytics 一致。 |

#### 修正

* 修正全螢幕模式中即時報告文字顯示的問題。(AN-183168)
* (**2019年月20日更新)**&#x200B;資料收集現在會拒絕包含「@」標誌的重新導向URL，防止攻擊者透過列入白名單的網域重新導向至惡意網站。
* (**2019年月20日更新)**&#x200B;現在所有來自不支援SameSite Cookie屬性且點擊包含第三方Cookie的瀏覽器，訪客移轉現在都停用。
* (**更新日期：2019**&#x200B;年月20日)修正第一次點擊未傳送新訪客的s_ vi Cookie問題。
...
Digital Publishing Suite Classic (DPSC) 即將於 2019 年 8 月 31 日終止服務。For more information, see the [Digital Publishing Suite Classic End-of-Life FAQ](https://helpx.adobe.com/digital-publishing-suite/help/eol-statement-for-dpsc.html).
