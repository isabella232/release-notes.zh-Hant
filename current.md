...
* (廣告商與Advertising Cloud轉換追蹤服務)搭配Apple的智慧追蹤防範(ITP)2.2，在月發佈的Advertising Cloud轉換追蹤Cookie會在24小時後自動從Apple Safari瀏覽器刪除。Advertising Cloud有新的ITP解決方案，可讓您追蹤在原始點按後超過24小時內Safari內發生的轉換。此解決方案使用本機儲存與iframe技術。如需實作指示，請洽詢您的Advertising Cloud Search帳戶管理員。
* 在搜尋&gt;進階(ACM)中，您現在可以為Google文字廣告和購物廣告範本設定促銷活動層級的最終URL字尾。
* 具有符合「客戶符合」資格之Google廣告帳戶的廣告商現在可以執行下列作業：
   * 從Adobe受眾區隔使用使用者ID建立Google廣告客戶比對對象。若要查看此功能，廣告商帳戶必須設定為允許。
   * 上傳客戶資料檔案以建立Google廣告客戶比對對象。檔案可以包含聯絡資訊(電子郵件地址、郵寄地址或電話號碼)、使用者ID或行動裝置ID。某些類型的聯絡資訊必須使用SHA-256演算法雜湊。
   * 更新任何Google客戶比對對象，除了Adobe觀眾建立的觀眾之外。您可以上傳資料以新增、刪除或取代對象的所有現有資料。所有聯絡資訊都必須使用SHA-256演算法雜湊。
* 「對象&gt;目標和對象&gt;排除」檢視包含「類型」欄。
